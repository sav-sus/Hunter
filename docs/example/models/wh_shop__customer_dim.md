...
| Worth fixing (silenced) | wh_shop__customer_dim has no description | Nobody reading the catalogue can tell what customers is for or whether it is the right table to use. 2 report fields depend on it. | models/warehouse/wh_shop/wh_shop__customer_dim.sql |
