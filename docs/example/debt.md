...
| Worth fixing | wh_shop__supplier_dim is designed but no model builds it | suppliers was designed and agreed, and nothing in the repository produces it yet. It is on the plan and not started. | analytics_warehouse/docs/data_model_design/physical_model.dbml |
| Tidy up | wh_shop__forecast_fact is built but switched off | The code for forecasts exists and is not running, so nothing is being produced from it. Either it is waiting to be turned on, or it was left behind and should be removed. | analytics_warehouse/docs/data_model_design/physical_model.dbml |
