...
Nothing is fetched from anywhere. The diagram library is bundled into the file at a pinned version, so the page opens with no network. With scripting off, the diagram source is shown as text with a note saying why, and nothing else on the page depends on it.

Two scripts run on the page. One filters the table list as you type; every row is already in the HTML, so with scripting off the list still reads in full. The other draws the model diagrams with Mermaid, which is the one library bundled into the file (pinned by version). With scripting off the diagram source is shown as text and the rest of the page is unaffected.
